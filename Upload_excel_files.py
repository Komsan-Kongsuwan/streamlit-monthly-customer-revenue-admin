...
# ---------------- Streamlit UI ----------------
st.set_page_config(page_title="Customer Revenue Dash", layout="wide")
...
if not all_data:
    status.text("⚠️ You uploaded wrong files, please upload correct files again (Browse files).")
    st.stop()
...
# ... (all your existing code unchanged until the end)

# store results in session_state for other pages
st.session_state["official_data"] = df_final.copy()   # ✅ standardized
st.session_state["customer_data_raw"] = df_final
st.session_state["customer_report_pivot"] = pivot_df
